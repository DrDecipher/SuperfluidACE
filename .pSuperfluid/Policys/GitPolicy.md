...
### Upstream Codex CLI Version Numbers

Upstream OpenAI’s `main` branch keeps `codex-cli/package.json` at
`"0.0.0-dev"`. A timestamped build number (e.g. `0.1.2505172129`) is **only**
inserted by their `codex-cli/scripts/stage_release.sh` helper when staging an
npm release — the change is written into a temporary directory and never
committed back to Git.  Consequently, building Codex CLI directly from the
repository will always display `v0.0.0-dev`.

If you need a local build that reports the real release version:

```
pnpm --filter codex-cli stage-release          # or pnpm run stage-release within codex-cli
# follow the script’s instructions; use the staged directory for testing/publish
```

Do **not** bump the version in `package.json` inside the repository – that
breaks upstream alignment and defeats their release workflow.